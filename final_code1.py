                        ...
                        if (a[x][y][1] == 0):
                            if ([x, y] == oneway[m][0]):
                                ...
                                if (oneway[m][1] == 1 and ((11-j)*12+12-i) != p[m]+12):
                                    if (x == 7 and y == 4):
                                        print(
                                            "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx2")
                                    a[x][y][0] = a[i][j][0]+b[x][y]
                                    a[x][y][1] += 1
                                    a[x][y][2] = (11-j)*12 + 12-i
                                    c.append([x, y])
                                if (oneway[m][1] == 2 and ((11-j)*12+12*i) != p[m]-1):
                                    if (x == 7 and y == 4):
                                        print(
                                            "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx3")
                                    a[x][y][0] = a[i][j][0]+b[x][y]
                                    a[x][y][1] += 1
                                    a[x][y][2] = (11-j)*12 + 12-i
                                    c.append([x, y])
                                if (oneway[m][1] == 3 and ((11-j)*12+12-i) != p[m]-12):
                                    print(
                                        "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxoneway[m][1] =3")
                                    a[x][y][0] = a[i][j][0]+b[x][y]
                                    a[x][y][1] += 1
                                    a[x][y][2] = (11-j)*12 + 12-i
                                    c.append([x, y])
...
# 			print(corners,ax,ay)
        ...
        if abs(path[j+1][0]-ax) < 25 and abs(path[j+1][1]-ay) < 25:
            j += 1
    ...
    for contour in Contours:
        ...
        if len(approx) == 4:
            ...
            if graph[y][x] == 7:
                ...
                # sh = [[y,x],[cx,cy]]
                print("sh:", sh)
                cv2.putText(img, "sh", (cx, cy),
                            cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
            elif graph[y][x] == 6:
                sp.append([[y, x], [cx, cy]])
                # sp = [[y,x],[cx,cy]]
                print("sp:", sp)
                cv2.putText(img, "sp", (cx, cy),
                            cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
        elif len(approx) > 4:
            ...
            if graph[y][x] == 7:
                ...
                # ch = [[y,x],[cx,cy]]
                print("ch:", ch)
                cv2.putText(img, "ch", (cx, cy),
                            cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))
            elif graph[y][x] == 6:
                cp.append([[y, x], [cx, cy]])
                # cp = [[y,x],[cx,cy]]
                print("cp:", cp)
                cv2.putText(img, "cp", (cx, cy),
                            cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0))


graph = np.zeros(144, np.int32).reshape(12, 12)
lower = np.array([[222, 222, 222], [0, 222, 0], [0, 222, 222], [
                 0, 0, 140], [0, 86, 0], [205, 105, 205], [220, 220, 0]])
upper = np.array([[232, 232, 232], [5, 232, 5], [5, 232, 232], [
                 5, 5, 150], [5, 96, 5], [216, 120, 216], [235, 235, 0]])
lower_blue = np.array([50, 0, 0])
upper_blue = np.array([255, 10, 10])
trace1 = []
...
if __name__ == "__main__":
    ...
    trace2 = dijkstra(graph, initial, pink[0][0])
    ...
    if len(trace1) <= len(trace2):
        ...
        troy = pink[0]
        troy1 = pink[1]
    env.remove_cover_plate(troy[0][0], troy[0][1])
    img1 = env.camera_feed()
    cv2.imwrite("arena.png", img1)
    img = cv2.imread("arena.png")
    detectHP(img)
    move([getPath(lamb)[-1], troy[1]])
    ########################
    ...
    if (len(cp) == 1):
        if (troy[0][0] == cp[0][0][0] and troy[0][1] == cp[0][0][1]):
            dest = ch[0][0]
    if (len(sp) == 1):
        if (troy[0][0] == sp[0][0][0] and troy[0][1] == sp[0][0][1]):
            dest = sh[0][0]

    trace1 = dijkstra(graph, troy[0], dest)
    print(trace1)
    move(getPath(trace1))
    move(getPath([trace1[-1], dest]))
    print(trace1)
    trace1 = dijkstra(graph, dest, troy1[0])
    move(getPath(trace1))
    env.remove_cover_plate(troy1[0][0], troy1[0][1])
    img1 = env.camera_feed()
    cv2.imwrite("arena.png", img1)
    img = cv2.imread("arena.png")
    detectHP(img)
    move([getPath(trace1)[-1], troy1[1]])
    if (len(cp) == 1):
        if (troy1[0][0] == cp[0][0][0] and troy1[0][1] == cp[0][0][1]):
            dest = ch[0][0]
    if (len(cp) == 2):
        if (troy1[0][0] == cp[0][0][0] and troy1[0][1] == cp[0][0][1]):
            dest = ch[0][0]
        if (troy1[0][0] == cp[1][0][0] and troy1[0][1] == cp[1][0][1]):
            dest = ch[0][0]
    if (len(sp) == 1):
        if (troy1[0][0] == sp[0][0][0] and troy1[0][1] == sp[0][0][1]):
            dest = sh[0][0]
    if (len(sp) == 2):
        if (troy1[0][0] == sp[0][0][0] and troy1[0][1] == sp[0][0][1]):
            dest = sh[0][0]
        if (troy1[0][0] == sp[1][0][0] and troy1[0][1] == sp[1][0][1]):
            dest = sh[0][0]
    trace1 = dijkstra(graph, troy1[0], dest)
    print(trace1)
    move(getPath(trace1))
    move(getPath([trace1[-1], dest]))
    cv2.imshow("img", img)
